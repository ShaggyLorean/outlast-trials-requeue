...
if grep -qE "$FORBIDDEN" <<< "$IMPORTS"; then
    grep -E "$FORBIDDEN" <<< "$IMPORTS" >&2
    echo "Forbidden focus, global-input, cursor, or display API imported" >&2
    exit 1
fi

grep -q 'PostMessageW' <<< "$IMPORTS"
grep -qE 'Subsystem[[:space:]]+00000002[[:space:]]+\(Windows GUI\)' <<< "$IMPORTS"
...
if grep -nE 'SetWindowPos\(game_window|ShowWindow\(game_window' "$SOURCE"; then
